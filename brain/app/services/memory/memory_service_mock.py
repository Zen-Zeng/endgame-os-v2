...
class MemoryService:
    ...
    def __init__(
        ...
        # 使用模拟版本的神经处理器
        from ..neural.processor_mock import NeuralProcessor
        self.neural_processor = NeuralProcessor()
        logger.info("记忆服务初始化成功（使用模拟神经处理器）")
                ...
                try:
                    ...
                    for subject, relation, obj in neural_result.get('triplets', []):
                        ...
                        if relation in ["完成", "贡献于"]:
                            # 创建项目节点
                            project_id = f"project_{obj}_{processed_count}_{hash(obj)}"
                            ...
                            # 创建目标节点
                            goal_id = f"goal_{obj}_{processed_count}_{hash(obj)}"
                            if self.graph_store.add_goal(
                                goal_id=goal_id,
                                title=obj
                            ):
                                # 建立项目-目标关系
                                project_id = f"project_{subject}_{processed_count}_{hash(subject)}"
                                if self.graph_store.add_project(
                                    project_id=project_id,
                                    name=subject
                                ):
        ...
        try:
            ...
            for subject, relation, obj in neural_result.get('triplets', []):
                ...
                if relation in ["完成", "贡献于"]:
                    # 创建项目节点
                    import time
                    project_id = f"project_{obj}_{processed_count}_{int(time.time() * 1000)}"
                    if self.graph_store.add_project(
                        project_id=project_id,
                        name=obj
                    ):
                    ...
                    # 创建目标节点
                    import time
                    goal_id = f"goal_{obj}_{processed_count}_{int(time.time() * 1000)}"
                    if self.graph_store.add_goal(
                        goal_id=goal_id,
                        title=obj
                    ):
                        # 建立项目-目标关系
                        project_id = f"project_{subject}_{processed_count}_{int(time.time() * 1000)}"
                        if self.graph_store.add_project(
                            project_id=project_id,
                            name=subject
                        ):
            ...
            # 确保有文本块才处理
            if not chunks:
                logger.warning("文本分块结果为空，跳过向量存储")
                return {
                    'success': False,
                    'error': '文本分块结果为空'
                }
