...
from app.services.memory.memory_service_mock import MemoryService
import logging
...
def test_pipeline():
    ...
    # 1. 测试文件导入
    test_file = "/Users/andornot/endgame-os-v2/brain/test_conversation_small.json"
    ...
    # 4. 测试文本添加
    test_text = "我完成了 PyTorch 模型优化，这个项目属于 Endgame OS 的性能优化目标。"
    ...
    query_results = memory_service.query_memory("PyTorch")
    logger.info(f"查询到 {len(query_results)} 个结果:")
    for i, res in enumerate(query_results):
        logger.info(f"结果 {i+1} [{res.get('type')}]: {res.get('content')[:100]}...")
